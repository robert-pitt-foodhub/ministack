...
### Added
- **CloudFormation — `AWS::Lambda::EventInvokeConfig` support** — CDK stacks that configure asynchronous Lambda retry limits, maximum event age, or success/failure destinations no longer roll back with `Unsupported resource type`. Configurations are isolated by function qualifier, mutable settings update in place, function/qualifier changes replace the resource, and stack deletion removes the configuration. Reported by @robert-pitt-foodhub (#1143).
